...
global $link;
$playerID = $_SESSION['raws']['email'];
$payment = mysqli_fetch_row($link->query("select id from payment where user = '$playerID'"));



$paymentForStudyMaterial = mysqli_fetch_row($link->query("select id from material where email = '$playerID'"));



if (empty($payment) || empty($paymentForStudyMaterial)) { ?>
    <p class="alert alert-danger text-center mt-5 pt-5 pb-5">You currently do not have access to this section. <br />
        Want access? <a href="<?= Inaki::path() ?>app/study?<?= time() ?>">click here</a></p>
<?php }
?>
<style type="text/css">
    .hd {
...
<div class="container <?php if (empty($payment) || empty($paymentForStudyMaterial)) {
                            echo "d-none";
                        } ?>" data-ref="<?= token(); ?>">
